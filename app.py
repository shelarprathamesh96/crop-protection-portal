...
# # Crop Protection Portal Center Login
...
def fertilizer_info():
    ...
    if request.method == 'POST':
        ...
        return render_template('fertilizer.html', protection=protection, protection_len=len(protection), display=True,
                               crops=crops, crop_len=len(crops))
...
def register():
    if request.method == 'POST':
        ...
        print(first_name, middle_name, last_name, phone_number, kisan_id, state, city, fullAddress, locality, password,
              conform_password)
...
def submit_issue(user_id, data_id):
    docs = db.collection(u'issue').document(u'' + user_id).collection(u'user_issue').document(
        u'' + data_id).get().to_dict()
    print(docs)
    ...
    if request.method == 'POST':
        ...
        account_sid = 'ACb69e66ee2d64eadd0f57811184c99bea'
        auth_token = 'c0f032f04f3be958174b075a1ed68eee'
